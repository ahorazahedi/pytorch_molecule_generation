...
class DataProcesser:
    ...
    def get_dataset_dims(self) -> None:
        ...
        self.dims["APDs"]  =  [np.prod(C.dim_f_add) + np.prod(C.dim_f_conn) + 1]
    ...
    def get_n_subgraphs(self) -> int:
        ...
        print("Calculation Number of SubGraphs")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process SMILES data.")
    
    parser.add_argument('--path', type=str, required=True, help='Path to the .smi file')
    parser.add_argument('--is_training_set', action='store_true', help='Flag to indicate if the file is a training set')

    args = parser.parse_args()

    print("======Start Data Processing======")
    
    data_processor = DataProcesser(path=args.path, is_training_set=args.is_training_set)
